...
나는 사내에서 URL 데이터를 처리하는 프로젝트를 맡고 있었다. 데이터의 빠른 검색을 위해 Elasticsearch를 테스트해보았는데, 일부 단어만으로 URL을 검색했을 때 잘 안 되는 것을 발견했다.
...
여기서 눈여겨보아야 할 부분은 [토크나이저(Tokenizer)](https://www.elastic.co/guide/en/elasticsearch/reference/current/analysis-tokenizers.html)다. 토크나이저는 입력된 텍스트를 '토큰'이라고 하는 개별 단어들로 분할한다. 이 작업을 토큰화(Tokenization)라고 한다. 이 과정을 통해 사용자는 구문에 포함된 일부 단어만 입력해도 구문을 검색할 수 있는 전문 검색이 가능하게 된다.
