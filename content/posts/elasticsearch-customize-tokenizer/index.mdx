---
title: 👨‍🏭 Elasticsearch에서 Tokenizer를 커스터마이징해보자 
date: 2021-04-02
...
해당 검색의 원인을 알기 위해서는 Elasticsearch가 데이터를 어떻게 저장하는지를 알아야 한다. Elasticsearch는 텍스트를 저장할 때, '[텍스트 분석(Tesxt Analysis)](https://esbook.kimjmin.net/06-text-analysis/6.2-text-analysis)'을 수행한다.  이는 Elasticsearch의 특징인 '[역 색인(Inverted index)](https://www.slideshare.net/kjmorc/ss-80803233)' 구조로 데이터를 저장하기 위함이다.
...
Elasticsearch는 기본적으로 [표준 분석기(Standard analyzer)](https://www.elastic.co/guide/en/elasticsearch/reference/master/analysis-standard-analyzer.html)를 제공하며, 그 안에는 [표준 토크나이저(Standard tokenizer)](https://www.elastic.co/guide/en/elasticsearch/reference/current/analysis-standard-tokenizer.html)가 있다.  예를 살펴보자. 
...
위와 같이 텍스트가 저장된다고 하면 표준 토크나이저는 아래와 같이 분리한다. 표준 토크나이저의 분할 기준은 크게 공백(Whitespace) 및 "@"과 같은 일부 특수문자다. 
...
아마 표준 토크나이저가 `://`또한 구분자로 인식했던 것 같다. 하지만 `.`은 아쉽게도 구분자로 인식하지 않는다. 그래서 'welldnn.com'으로 검색하면 결과가 나오지 않았던 것이다.
...
분할되는 N-gram의 수가 많아 몇몇 구간은 생략했다. 분석된 것을 보면 최소 2부터 최대 30길이까지 분할한다. 생성된 토큰에는 'welldnn'과 'welldnn.com'이 들어 있음을 확인할 수 있다.
