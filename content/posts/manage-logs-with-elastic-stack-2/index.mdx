...
이제 Stack에서 다루려는 데이터를 산지 직송해주는 경량 데이터 전달자 Beats를 설치할 차례다.

이번 글에서는 로그(Log) 수집기인 Filebeat를 설치하고, 애플리케이션에 부착하여 로그를 수집하고, 그 결과물을 Kibana에서 확인하는 과정을 기술한다.
...
Filebeat는 로그 파일이나 사용자가 지정한 장소를 관찰한다. 그런 다음 로그 이벤트를 수집해서 색인화(저장)를 위해 Elasticsearch나 Logstash로 보낸다. 
...
모니터링하려는 애플리케이션의 서버에 설치해야 한다. Filebeat는 운영 환경에 따라 [다양한 설치 방법](https://www.elastic.co/guide/en/beats/filebeat/7.12/filebeat-installation-configuration.html#other-installation-options)을 제공한다.

나는 앞서 도커 이미지로 설치했던 Elasticsearch, Kibana와 달리 Filebeat는 애플리케이션 서버가 있는 장비에 직접 설치했다. (이 방법이 제일 간단해 보였다...)
...
파일 비트를 시작하기 전에 몇 가지 설정이 필요하다. 설정은 `filebeat.yml` 파일을 이용한다. 만약 예시를 보고 싶다면 `filebeat.reference.yml`을 참고하면 된다.
...
추후 기본적으로 제공하는 대시보드 구성을 통해 로그 데이터들을 시각화하여 효과적으로 데이터를 관리할 수도 있다.
