...
이러한 로그들은 추후 이슈가 발생했을 때 원인을 파악하기 위해 가장 먼저 확인하는 데이터다. 신속하게 해결하려면 빠르게 확인할 수 있어야 하고, 특정 항목/값을 기준으로 알맞게 검색할 수 있어야 한다. 
...
> 💡 `Elastic Stack`이 수행하는 더 자세한 기능들은 [여기를](https://www.elastic.co/kr/elastic-stack) 참고할 수 있다.
...
Elasticsearch는 Stack의 핵심기능이다. Apache Lucene을 기반으로 구축된 분산형 RESTful 검색 및 분석 엔진으로써 다양한 형식의 문서 파일을 검색하고 색인화할 수 있다.
...
나는 로그 데이터를 전송하기 위해 로그 파일 전용 수집기인 [Filebeat](https://www.elastic.co/kr/beats/filebeat)를 사용했다.
...
각각의 과정들을 담당하는 제품들은 다음과 같다.

![elastic_stack_flow](./elastic_stack_flow.png)
...
- Kibana: 로그 데이터의 검색 및 관리

### with Docker

간편한 구현과 세부 설정을 위해 Elastic에서 제공하는 Docker image를 이용해 Elasticsearch와 Kibana를 설치했다.

- [Docker를 통한 설치 가이드](https://www.elastic.co/guide/en/elastic-stack-get-started/current/get-started-docker.html)
...
Kibana 컨테이너의 설정을 하는 부분에서 `ELASTICSEARCH_HOSTS` 필드는 Elasticsearch 노드들 각각의 IP 주소를 입력한다. 나는 노드 별로 IP 주소를 생성하지 않을 것이기에 기본 Elasticsearch 접근 주소만 입력했다. 
...
- [Running the Elastic Stack on Docker | Getting Started [7.12] | Elastic. (2021.04.14)](https://www.elastic.co/guide/en/elastic-stack-get-started/current/get-started-docker.html)
