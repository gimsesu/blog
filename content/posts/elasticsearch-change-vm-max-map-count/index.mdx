...
리눅스 환경에서 Elasticsearch가 정상적으로 작동하기 위해서는 `vm.max_map_count` 값을 변경해야 한다. 

`vm.max_map_count`는 무엇이고, 왜 이 값을 변경해야 하는지 알아보자.
...
Docker를 이용해 Elasticsearch를 설치했으나 실행되지 않았다. 해당 도커 컨테이너의 로그를 확인하니 이런 에러가 떴다. 
...
```
 ...
 [Bootstrap 검사 단계](https://www.elastic.co/guide/en/elasticsearch/reference/7.12/bootstrap-checks.html)에서 `vm.max_map_count`의 값이 부족하니 상향하라는 내용이었다.

###  올리지 뭐

`vm.max_map_count`의 기본값은 Linux의 경우 `65,530`이다. Elasticsearch를 실행하기 위해서는 최소 `262,144`로 [상향하는 것을 권장한다.](https://www.elastic.co/guide/en/elasticsearch/reference/current/docker.html#_set_vm_max_map_count_to_at_least_262144)
...
친절한 에러 메시지 덕에 수월하게 문제를 해결했다. 하지만 호기심이 남는다. `vm.max_map_count`는 무엇이고, 왜 수정해야 하는 걸까? 🕵🏼
...
>
> 대부분의 응용 프로그램에는 천 개 미만의 맵이 필요하지만 특정 프로그램(특히 malloc debugger)은 더 많은 맵을 소비할 수 있습니다.
> 예: 할당 당 최대 1개 또는 2개의 맵
>
> 기본값은 65,536입니다.

`vm.max_map_count` 변수는 `/proc/sys/vm/max_map_count` 파일을 가리킨다. 이 파일은 Linux kernel의 메모리 맵* 영역의 최대 개수를 조작할 수 있다. 
...
- Elasticsearch는 자신의 가상 주소 공간에 특정 파일을 매핑하는 작업이 빈번하게 일어나는 듯 하다.  

## Virtual memory

Elasticsearch를 프로덕션 레벨에서 사용하려 한다면 실행 전 [충분한 가상 메모리의 확보를 권장](https://www.elastic.co/guide/en/elasticsearch/reference/current/vm-max-map-count.html)하며 그 방법으로 `vm.max_map_count`의 조작법을 알려준다.
...
그리고 이 방법을 언급하는 과정에서 낯선 용어인 '[`mmapfs`](https://www.elastic.co/guide/en/elasticsearch/reference/current/index-modules-store.html#mmapfs) 디렉터리'와 '`mmap`*'이 나온다.

위 인용문에서는 `mmap` 개수의 제한이 너무 낮다'라고 했다. `mmap`은 메모리 매핑을 실행하고, 그 결과물은 [메모리 맵 파일(memory mapped file)](https://ko.wikipedia.org/wiki/%EB%A9%94%EB%AA%A8%EB%A6%AC_%EB%A7%B5_%ED%8C%8C%EC%9D%BC)이다.

💡 [`mmap`](https://en.wikipedia.org/wiki/Mmap): `유닉스(Unix) 시스템 호출(System call. syscall)로서, 메모리 매핑을 실행하는 함수. `  

### 🕵🏼 추측 2

- Elasticsearch가 `mmapfs` 디렉터리를 이용하여 [인덱스(Index)](https://esbook.kimjmin.net/03-cluster/3.2-index-and-shards)를 저장하는 과정에서 `mmap`을 이용한 빈번한 메모리 매핑 작업이 발생하고, 그 결과로 많은 메모리 맵 파일이 생길 수 있다.

- 생성되는 메모리 맵 파일의 수는, Linux의 경우 기본적으로 정해놓은 `max_map_count`가 Elasticsearch 관점에서는 매우 적기 때문에 수정을 해야 한다.

## mmapfs

Elasticsearch 문서에 기술해 놓은 `mmapfs`의 설명이다.
...
> MMap FS 유형은 파일을 메모리에 매핑하여(mmap) 파일 시스템 (Lucene의 `MMapDirectory`에 대응한다.)에 샤드 인덱스를 저장한다. 메모리 매핑은 매핑되는 파일과 동일한  크기의 프로세스 내 가상 메모리 주소 공간의 일부를 사용한다. 이 클래스를 사용하려면 충분한 가상 주소 공간을 허용했는지 확인해야 한다. 

여기서 `mmapfs`는 Elasticsearch의 인덱스를 저장하고 접근하는 [여러 파일 시스템 구현 유형들](https://www.elastic.co/guide/en/elasticsearch/reference/current/index-modules-store.html) 중 하나라는 사실을 알 수 있다.  

또 'file system'을 언급하는 부분에서 'Lucene의 `MMapDirectory`'가 언급된다. 

`mmapfs`는 Elasticsearch에서 인덱스를 저장하는 파일 시스템 구현 유형 중의 하나다. `MMapDirectory` 말고는 달리 알아낸 새로운 사실이 없다.

### 🕵🏼 추측 3

- Elasticsearch가 `mmapfs`  저장소 유형을 이용해 [인덱스(Index)](https://esbook.kimjmin.net/03-cluster/3.2-index-and-shards)를 저장하는 과정에서 `mmap`을 이용한 빈번한 메모리 매핑 작업이 발생하고, 그 결과로 많은 메모리 맵 파일이 생길 수 있다.

- 그러나 자세한 저장 원리는 Elasticsearch 공식 문서에 더 이상 나와 있지 않다. 

- mmapfs는 Lucene의 `MMapDirectory`에 대응하는 파일 시스템 구현이다. MMapDirectory가 어떻게 인덱스를 저장하고 접근하는지 알면 메모리 매핑이 많이 일어나는 이유를 알 수 있을 것이다. 

## MMapDirectory

Elasticsearch는 정보검색 라이브러리인 [Lucene](https://lucene.apache.org/core/)을 기반으로 만들어졌다. [`MMapDirectory`](https://lucene.apache.org/core/6_3_0/core/org/apache/lucene/store/MMapDirectory.html)는 Lucene의 디렉터리 구현이다.

Elasticsearch는 검색 요청이 들어오면 저장된 인덱스 파일에 빠르게 접근해야 한다. 이를 위해 Elasticsearch는 [메모리 매핑](https://ko.wikipedia.org/wiki/%EB%A9%94%EB%AA%A8%EB%A6%AC_%EB%A7%B5_%ED%8C%8C%EC%9D%BC) 기법을 이용하여 인덱스를 관리한다.

책  『Elasticsearch in Action』에서 쓰인 `MMapDirectory` 설명을 참고해보자.
...
이처럼 Elasticsearch는 운영 체제를 이용하여 인덱스 파일들을 메모리에 가상으로 매핑하고, 파일 시스템 캐시*를 생성한다.

💡  [`파일 시스템 캐시(File System Cache)`](https://docs.oracle.com/cd/E19424-01/820-4811/anobm/index.html): `최근에 디스크에서 읽은 데이터들을 보관한다. 최근에 디스크에서 읽은 데이터를 보관하기 때문에 이후 응용 프로그램은 디스크에서 데이터를 다시 읽을 필요가 없고 캐시에서 데이터를 가져올 수 있다.`
...
Elasticsearch에서 사용하는 저장소 유형은 [`hybridfs`](https://www.elastic.co/guide/en/elasticsearch/reference/current/index-modules-store.html#hybridfs)다. `hybridfs`는 운영 체제에 따라 `mmapfs`와 [`noifs`](https://www.elastic.co/guide/en/elasticsearch/reference/current/index-modules-store.html#niofs)를 적절히 선택하여 사용한다. 따라서 Elasticsearch를 설치할 때 `hybridfs`나 `mmapfs`를 사용한다면 `vm.max_map_count` 사전에 설정해주어야 한다.
...
'Elasticsearch 문서에서 이거 수정하라고 했어요. `vm.max_map_count` 값을 수정하면 돼요!' 라고 간단히 쓰고 끝내기엔 내가 애매하게 알거나 모르는 개념들이 너무 많았다. 

좋은 자료들을 찾아도 해석 실력이 부족하거나, 해석을 해도 그걸 알아들을 만큼의 배경 지식이 부족해 결국 인용하지 못한 자료들도 있었다.
