...
- Elasticsearch는 자신의 가상 주소 공간에 특정한 파일을 매핑하는 작업이 많이 일어나는 듯하다.
...
여기서 mmapfs는 Elasticsearch의 인덱스를 저장하고 접근하는 [여러 파일 시스템 구현](https://www.elastic.co/guide/en/elasticsearch/reference/current/index-modules-store.html) 중 하나라는 사실을 알 수 있다.
...
Elasticsearch는 정보검색 라이브러리인 [Lucene](https://lucene.apache.org/core/)을 기반으로 만들어졌다. [MMapDirectory](https://lucene.apache.org/core/6_3_0/core/org/apache/lucene/store/MMapDirectory.html)는 Lucene의 인덱스를 저장 및 관리하는 디렉터리 구현이다.

그리고 Elasticsearch의 mmapfs은 MMapDirectory를 사용하여 인덱스를 다루는 형식인 것이다. 
...
이처럼 Elasticsearch는 운영 체제를 이용하여 인덱스 파일들을 메모리에 가상으로 매핑하고 파일 시스템 캐시*를 생성한다. 이 덕분에 검색 요청이 들어오면 캐시에 들어 있는 신속하게 인덱스 파일에 접근해 응답을 제공한다.
...
[Bootstrap 검사 단계](https://www.elastic.co/guide/en/elasticsearch/reference/7.12/bootstrap-checks.html)에서 

Elasticsearch에서 사용하는 저장소 유형은 [`hybridfs`](https://www.elastic.co/guide/en/elasticsearch/reference/current/index-modules-store.html#hybridfs)다. hybridfs는 운영 체제에 따라 mmapfs와 [`noifs`](https://www.elastic.co/guide/en/elasticsearch/reference/current/index-modules-store.html#niofs)를 적절히 선택하여 사용한다. 따라서 Elasticsearch를 설치할 때 `hybridfs`나 `mmapfs`를 사용한다면 `vm.max_map_count` 사전에 설정해주어야 한다.
