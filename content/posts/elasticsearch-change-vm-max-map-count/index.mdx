...
- Elasticsearch는 자신의 가상 주소 공간에 특정 파일을 매핑하는 작업이 많이 일어나는 듯하다.
...
- Elasticsearch가 `mmapfs` 디렉터리를 이용하여 [인덱스(Index)](https://esbook.kimjmin.net/03-cluster/3.2-index-and-shards)를 저장하는 과정에서 `mmap`을 이용한 많은 수의 메모리 매핑 작업이 발생할 수 있고, 그 결과로 많은 메모리 맵 파일이 생성될 수 있다.
...
- Elasticsearch가 `mmapfs` 저장소 유형을 이용해 [인덱스(Index)](https://esbook.kimjmin.net/03-cluster/3.2-index-and-shards)를 저장하는 과정에서 `mmap`을 이용한 많은 수의 메모리 매핑 작업이 발생할 수 있고, 그 결과로 많은 메모리 맵 파일이 생성될 수 있다.
